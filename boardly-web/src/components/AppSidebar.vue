...
interface MenuItem {
  ...
  to: string
  roles: ('ADMIN' | 'MEMBER')[]
  icon: 'dashboard' | 'boards' | 'tasks' | 'sprints' | 'backlog' | 'team' | 'billing' | 'settings'
}

interface MenuSection {
  titleKey: 'section.core' | 'section.agile' | 'section.admin'
  items: MenuItem[]
}
...
const sections: MenuSection[] = [
  ...
  {
    titleKey: 'section.agile',
    items: [
      { labelKey: 'nav.sprints', to: '/sprints', roles: ['ADMIN', 'MEMBER'], icon: 'sprints' },
      { labelKey: 'nav.backlog', to: '/backlog', roles: ['ADMIN', 'MEMBER'], icon: 'backlog' },
    ],
  },
  {
    titleKey: 'section.admin',
    items: [
      { labelKey: 'nav.team', to: '/team', roles: ['ADMIN', 'MEMBER'], icon: 'team' },
      { labelKey: 'nav.billing-subscription', to: '/billing', roles: ['ADMIN', 'MEMBER'], icon: 'billing' },
      { labelKey: 'nav.settings', to: '/settings', roles: ['ADMIN', 'MEMBER'], icon: 'settings' },
    ],
  },
]

const iconPaths: Record<NonNullable<MenuItem['icon']>, string> = {
  dashboard: 'M3 12l9-9 9 9M5 10v10h14V10',
  boards: 'M4 5h16v14H4zM9 5v14M15 5v14',
  tasks: 'M9 11l3 3L20 6M4 6h3M4 12h3M4 18h10',
  sprints: 'M4 7h16M4 12h10M4 17h6M16 11l2 2 3-3',
  backlog: 'M6 5h12M6 10h12M6 15h12M6 20h8',
  ...
  billing: 'M3 8h18v10H3zM7 5h10M7 13h5',
  ...
  <aside id="top-bar-sidebar"
    ...
    <div class="flex flex-col justify-between h-full px-3 py-4 pt-20 overflow-y-auto bg-background border-e border-border">
      ...
      <nav class="flex-1 space-y-4">
        <section v-for="section in filteredSections" :key="section.titleKey">
          <p class="px-4 mb-2 text-xs font-semibold uppercase tracking-wide text-text/50">
            {{ t(section.titleKey) }}
          </p>
          <div class="space-y-2">
            <RouterLink
